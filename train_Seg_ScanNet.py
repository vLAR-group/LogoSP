...
def parse_args():
    ...
    ###
    parser.add_argument('--save_path', type=str, default='ckpt/ScanNet/seg',
                        help='model savepath')
    parser.add_argument('--max_epoch', type=int, default=200, help='max epoch')
    parser.add_argument('--max_iter', type=int, default=200*121, help='max iter')
    ###
    ...
    parser.add_argument('--distill_ckpt', type=str, default='./ckpt/ScanNet/distill/checkpoint_300.tar')
...
def cluster(args, logger, cluster_loader, current_sp, model):
    ...
    feats, labels, sp_index, context = get_sp_feature(args, cluster_loader, current_sp, model=model)
    sp_feats = torch.cat(feats, dim=0)
    sp_feats = F.normalize(sp_feats)
    print('clustering ...')
    if sp_feats.shape[0]>30000:
        print(sp_feats.shape)
        idx = np.random.choice(sp_feats.shape[0], 30000, replace=False)
        sampled_sp_feats = sp_feats[idx]
    else:
    ...
    if i<args.col_num:
        logger.info('The current spectral pattern numbers is: %.2f, less then predefined number', i, args.col_num)
        valid_eigvectors, valid_amp_vector = eigvectors, all_amp_vector
    else:
