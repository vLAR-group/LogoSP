...
def parse_args():
    ...
    ###
    parser.add_argument('--save_path', type=str, default='ckpt/S3DIS/seg2',
                        help='model savepath')
    parser.add_argument('--max_epoch', type=int, default=100, help='max epoch')
    parser.add_argument('--max_iter', type=int, default=2500, help='max iter')
    ###
    ...
    parser.add_argument('--eneg_ratio', type=int, default=0.99)
    parser.add_argument('--distill_ckpt', type=str, default='./ckpt/S3DIS/distill/checkpoint_700.tar')
...
def cluster(args, logger, cluster_loader, current_sp, model):
    ...
    ####################################################################################################################
    eigvectors = rbf_eig_vector(sampled_sp_feats.cuda())#(sp_feats)  ## spec_embedding is numpy, eigvectors are tensor on cpu
    ## select W
    ...
    #
    # spec_embedding = valid_eigvectors[:, 0:10].numpy()
    #################################################################################################################
