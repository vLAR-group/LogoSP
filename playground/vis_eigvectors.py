...
def map_color(err, norm=True, idx=None):
    ...
    if idx is None:
        ...
        if idx==0:
            ...
            dark_blue = (128, 128, 128)  # RGB for dark blue
            yellow = (102, 205, 102)  # RGB for yellow
        elif idx ==2:
            dark_blue = (128, 128, 128)  # RGB for dark blue
            yellow = (135, 206, 235)  # RGB for yellow
        elif idx ==3:
            dark_blue = (128, 128, 128)  # RGB for dark blue
            yellow = (255, 160, 170)  # RGB for yellow
...
# feat_path = '/home/zihui/SSD/DivSP/data/ScanNet/ours_60sp_0.05'
grow_sp_num = 30
segment_num = 20 ## segment number for each scene
feats_dim = 384
# train_scene_id = read_txt('/home/zihui/SSD/DivSP/data/ScanNet/scannet/scannet_3d/scannetv2_val.txt')#[100:150]
# train_scene_id = ['scene0005_00', 'scene0006_00', 'scene0011_00', 'scene0000_00']
train_scene_id = ['scene0297_00', 'scene0354_00', 'scene0420_00', 'scene0487_00', 'scene0587_00']
...
for scene_id in train_scene_id:
    ...
    ##
    raw_sp = np.load(os.path.join('/home/zihui/SSD/GOPS/data/scannet/processed/tmp', scene_id[5:]+'.npy'))[:, 9]
...
spec_embedding = spec_embedding.T  ## [N, K]
##
folder = 'vis_fused_eigvector_v5'
for idx, scene_id in enumerate(train_scene_id):
    print(scene_id)
    coords, sp, raw_coords = all_coords[idx], all_sp[idx], all_raw_coords[idx]

    for lamda_index in range(spec_embedding.shape[1]):
        cur_spec_embedding = spec_embedding[:, lamda_index]
        ##
        ...
        ###
        ''' vis eigvectors'''
        softmask = (cur_spec_embedding**2)  ## the eigvector for all datasets [N, 1]
        cur_eigvector = softmask[acc_sp[idx].unique().long()]
        rgb = map_color(cur_eigvector).numpy()  # *255
        rgb = rgb[sp]
        write_ply(os.path.join(save_path, 'spGFT_eigvector_' + str(lamda_index) + '_.ply'), [raw_coords, rgb[raw2vol].astype(np.uint8)], ['x', 'y', 'z', 'red', 'green', 'blue'])

        ''' vis hard eigvectors'''
        hardmask = (softmask > softmask.mean()*3).float() ##3 for scene0005_00;
        cur_eigvector = hardmask[acc_sp[idx].unique().long()]
        rgb = map_color(cur_eigvector, norm=False).numpy()  # *255
        rgb = rgb[sp]
        write_ply(os.path.join(save_path, 'spGFT_hard_eigvector_' + str(lamda_index) + '_.ply'), [raw_coords, rgb[raw2vol].astype(np.uint8)], ['x', 'y', 'z', 'red', 'green', 'blue'])