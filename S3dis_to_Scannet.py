...
def parse_args():
    ...
    parser.add_argument('--drop_threshold', type=int, default=10, help='mask counts')
...
def cluster(args,cluster_loader,model):
    ...
    primitive_labels = KMeans(n_clusters=args.primitive_num, n_init=10, random_state=0, n_jobs=-1).fit_predict(sp_feats.numpy())
...
if __name__ == '__main__':
    model_path = './ckpt/S3DIS/model_100_checkpoint.pth'

    args = parse_args()
    '''Setup logger'''
    if not os.path.exists(args.save_path):
        os.makedirs(args.save_path)
    logger = set_logger(os.path.join(args.save_path, 'S3dis_to_Scannet.log'))
    ...
    model= model.cuda()
    model.eval()
    ##cluster to get classifier
    all_areas = ['Area_1', 'Area_2', 'Area_3', 'Area_4', 'Area_5', 'Area_6']
    test_areas = ['Area_5']
    training_areas = sorted(list(set(all_areas) - set(test_areas)))
    logger.info(training_areas)
    clusterset = S3DIStrain(args,areas=training_areas)
    cluster_loader = DataLoader(clusterset, batch_size=10, shuffle=True, collate_fn=cfl_collate_fn(),num_workers=args.workers, pin_memory=True, worker_init_fn=worker_init_fn(seed))
    cluster_loader.dataset.mode = 'cluster'
    classifier = cluster(args,cluster_loader,model)

    ##start eval
    args.ignore_label = -1
    args.data_path = './data/ScanNet/processed'
    args.sp_path = './data/ScanNet/initial_superpoints_0.25'
