...
import numpy as np
import random
from datasets.S3DIS import S3DISdistill, cfl_collate_fn_distill
import torch
...
def parse_args():
    ...
    parser.add_argument('--feats_path', type=str, default='./data/S3DIS/DINOv2_feats_s14up4_voxel_0.05/',
                        help='project dino point feature')
    ###
    parser.add_argument('--save_path', type=str, default='ckpt/S3DIS/distill/',
                        help='model savepath')
    parser.add_argument('--max_epoch', type=int, default=700, help='max epoch')
    parser.add_argument('--max_iter', type=int, default=500*151, help='max iter')
    ###
...
def main(args, logger):
    ...
    model = Res16FPN18(in_channels=args.input_dim, out_channels=12, conv1_kernel_size=args.conv1_kernel_size, config=args)
    logger.info(model)
    model = model.cuda()
...
def train(train_loader, logger, model, optimizer, epoch, scheduler):
    ...
    for batch_idx, data in enumerate(train_loader):
        ...
        coords, features, labels, inverse_map, index, scenenames, feats_2d, indicator = data

        in_field = ME.TensorField(features, coords, device=0)
        feats_3d = model(in_field)
        feats_3d, feats_2d = feats_3d[indicator.squeeze(1)>=5], feats_2d[indicator.squeeze(1)>=5]
